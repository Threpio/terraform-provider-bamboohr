...
goimports:
	...
	goimports -local "github.com/hashicorp/terraform-provider-bamboohr" -w $(PKG_NAME)/
...
testacc: fmtcheck
	TF_ACC=1 go test $(TEST) -v $(TESTARGS) -timeout 180m -ldflags="-X=github.com/hashicorp/terraform-provider-bamboohr/version.ProviderVersion=acc"

acctests: fmtcheck
	TF_ACC=1 go test -v ./internal/services/$(SERVICE)/tests/ $(TESTARGS) -timeout $(TESTTIMEOUT) -ldflags="-X=github.com/hashicorp/terraform-provider-bamboohr/version.ProviderVersion=acc"
...
docs-lint:
	...
	@tfproviderdocs check -provider-name=bamboohr -allowed-guide-subcategories="Authentication,Upgrade Guides" -enable-contents-check -require-schema-ordering -require-guide-subcategory -require-resource-subcategory
